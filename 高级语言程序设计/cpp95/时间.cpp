...
/* 可以在此定义其它需要的函数 */
const int SECONDS_OF_COMMON_YEARS = 60 * 60 * 24 * 365;
const int SECONDS_OF_LEAP_YEARS = 60 * 60 * 24 * 366;
const int SECONDS_OF_28 = 60 * 60 * 24 * 28;
const int SECONDS_OF_29 = 60 * 60 * 24 * 29;
const int SECONDS_OF_30 = 60 * 60 * 24 * 30;
const int SECONDS_OF_31 = 60 * 60 * 24 * 31;
...
struct tj_time* tj_time_convert(int input_time)
{
	...
	int tmp = input_time + 8 * 60 * 60;
	result.tj_second = tmp % 60;
	result.tj_minute = tmp / 60 % 60;
	result.tj_hour = tmp / 3600 % 24;
	result.tj_year = 1970;
	while (tmp >= 0)
	{
		if ((result.tj_year % 4 == 0) && (result.tj_year % 100 != 0) || (result.tj_year % 400 == 0))
			tmp -= SECONDS_OF_LEAP_YEARS;
		else
			tmp -= SECONDS_OF_COMMON_YEARS;
		result.tj_year++;
	}
	if ((--result.tj_year % 4 == 0) && (result.tj_year % 100 != 0) || (result.tj_year % 400 == 0))
		tmp += SECONDS_OF_LEAP_YEARS;
		...
		tmp += SECONDS_OF_COMMON_YEARS;
	//result.tj_year--;
	int February = SECONDS_OF_28;
	if ((result.tj_year % 4 == 0) && (result.tj_year % 100 != 0) || (result.tj_year % 400 == 0))
		February = SECONDS_OF_29;
	int days[13] = { 0,SECONDS_OF_31,February,SECONDS_OF_31,
					   SECONDS_OF_30,SECONDS_OF_31,SECONDS_OF_30,
					   SECONDS_OF_31,SECONDS_OF_31,SECONDS_OF_30,
					   SECONDS_OF_31,SECONDS_OF_30,SECONDS_OF_31 };
	int i;
	for (i = 1; tmp >= 0; i++)
		tmp -= days[i];
	tmp += days[--i];
	result.tj_month = i;
	result.tj_day = tmp / 60 / 60 / 24 + 1;
	/* 实现过程结束 */
